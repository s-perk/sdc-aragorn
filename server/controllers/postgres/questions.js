...
module.exports = {
    ...
    client.query(queryStr)
      .then((data) => {
        ...
        for (var i = 0; i < data.rows.length; i++) {
          ...
          // handle if only one result or last result
  ...
  getWithPhotos: function (req, res) {
    ...
    var queryStr =
      `select questions.id as "question_id", questions.question_body, questions.instant, questions.asker_name, questions.asker_email, questions.reported, questions.helpful, \
      answers.id as "answer_id", answers.answer_body, answers.answerer_name, answers.answerer_email, answers.helpful as "answer_helpful", answers.reported as "answers_reported",  answers.instant as "answer_instant", answers.photos as "photos"\
      from questions\
      left outer join answers on (questions.id = answers.question_id)\
      where questions.product_id=${product_id} \
      `
